...
export const translateChannel = (element: HTMLElement, onComplete?: GSAPCallback, onRemove?: GSAPCallback) => {
  ...
  const deltaX = innerWidth <= 640 ? innerWidth * -0.7 : innerWidth <= 1024 ? innerWidth * -0.8 : innerWidth * -0.44;
  ...
  const deltaTop = newContainerRect.top * -0.5;
